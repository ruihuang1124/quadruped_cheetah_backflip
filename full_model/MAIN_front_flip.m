...
[t,X] = ode45(@(t,X)dyn_double_stance_front_flip(t,X,p),[tstart 0.5],ic, options);
...
Xout = [Xout; X(2:nt,:)];
for tstep = 2:nt
    [~,u,GRFF,GRFB] = dyn_double_stance_front_flip(t(tstep),X(tstep,:)',p);
    uout = [uout; u'];
...
[t,X] = ode45(@(t,X)dyn_front_stance_front_flip(t,X,p),[tout(end) tfinal],Xout(end,:),options);
...
Xout = [Xout; X(2:nt,:)];
for tstep = 2:nt
    [~,u,GRFF,GRFB] = dyn_front_stance_front_flip(t(tstep),X(tstep,:)',p);
    uout = [uout; u'];
    GRFFout = [GRFFout; GRFF'];
    GRFBout = [GRFBout; GRFB'];
end
