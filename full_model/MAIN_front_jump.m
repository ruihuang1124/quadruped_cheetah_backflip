...
% Double stance phase
options = odeset('Events',@(t,X)take_off(t,X,p),'MaxStep',1e-3);
[t,X] = ode45(@(t,X)dyn_double_stance_front_jump(t,X,p),[tstart 0.5],ic, options);
...
Xout = [Xout; X(2:nt,:)];
for tstep = 2:nt
    [~,u,GRFF,GRFB] = dyn_double_stance_front_jump(t(tstep),X(tstep,:)',p);
    uout = [uout; u'];
...
[t,X] = ode45(@(t,X)dyn_back_stance_front_jump(t,X,p),[tout(end) tfinal],Xout(end,:),options);
...
Xout = [Xout; X(2:nt,:)];
for tstep = 2:nt
    [~,u,GRFF,GRFB] = dyn_back_stance_front_jump(t(tstep),X(tstep,:)',p);
    uout = [uout; u'];
    GRFFout = [GRFFout; GRFF'];
    GRFBout = [GRFBout; GRFB'];
end
...
%% Aerial phase
options = odeset('Events',@(t,X)touch_down(t,X,p),'MaxStep',1e-3);
[t,X] = ode45(@(t,X)dyn_aerial_front_jump(t,X,p),[tout(end) tfinal],Xout(end,:),options);
...
Xout = [Xout; X(2:nt,:)];
for tstep = 2:nt
    [~,u,GRFF,GRFB] = dyn_aerial_front_jump(t(tstep),X(tstep,:)',p);
    uout = [uout; u'];
    GRFFout = [GRFFout; GRFF'];
    GRFBout = [GRFBout; GRFB'];
end
