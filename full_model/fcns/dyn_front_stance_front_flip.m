function [dXdt,u,GRFF,GRFB] = dyn_front_stance_front_flip(t,X,p)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
t

global FFy FBy

params = p.params;
q = X(1:7);
qdot = X(8:14);

De = fcn_De(q,params);
Ce = fcn_Ce(q,qdot,params);
Ge = fcn_Ge(q,params);
Be = fcn_Be(q,params);

% Holonomic constraints
JBtoe = fcn_JBtoe(q,params);
JdotBtoe = fcn_JdotBtoe(q,qdot,params);
JFtoe = fcn_JFtoe(q,params);
JdotFtoe = fcn_JdotFtoe(q,qdot,params);


%% Controller
% Feedforward force
s = t / 0.3;        % stance phase parametrization s = [0, 1]
% Force profile using Bezier polynomials
FFy = polyval_bz([0 20 40 60 130], s);
FFx = polyval_bz([0 -8 -10 -15 -30], s);

uF = -JFtoe'*[FFx; FFy];

qbh_TD = -1/3*pi; % Desired touchdown angle
qbk_TD = 0.5*pi;

ubh = 50*(qbh_TD-q(6)) + 2*(0-qdot(6));
ubk = 50*(qbk_TD-q(7)) + 2*(0-qdot(7));

if ubh > 15
    ubh = 15;
end
if ubh < -15
    ubh = -15;
end
if ubk > 15
    ubk = 15;
end
if ubk < -15
    ubk = -15;
end

u = [uF(4);uF(5);ubh;ubk];

% Solve the linear system:
% De * ddq + Ce * dq + Ge = JF' * FGRF + JB' * BGRF + Be * u (7 eqns)
% Jhc * ddq + dJhc * dq = 0 (2 eqns)
% [De  -JFtoe'] * [ddq ] = [Be*u - Ce*dq - Ge]
% [JFtoe  0   ]   [GRFF]   [-JdotFtoe*dq     ]
% unknowns: ddq(7x1), GRF(4x1) 
% control: u(4x1)
Amat = [De -JFtoe'; 
        JFtoe zeros(2,2)];
bvec = [Be*u - Ce*qdot - Ge; 
        -JdotFtoe*qdot];

ddqu = Amat \ bvec;
ddq = ddqu(1:7);
GRFB = [0;0];
GRFF = ddqu(8:9);

FFy = GRFF(2);
FBy = GRFB(2);

dXdt = [qdot; ddq];

end

